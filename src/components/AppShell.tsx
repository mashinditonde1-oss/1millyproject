...
import { Home, FileText, Users, BarChart2, MoreHorizontal } from "lucide-react";
import { cn } from "@/lib/utils";

const items = [
  { to: "/app", label: "Home", icon: Home, exact: true },
  { to: "/app/docs", label: "Docs", icon: FileText, exact: false },
  { to: "/app/clients", label: "Clients", icon: Users, exact: false },
  { to: "/app/analytics", label: "Analytics", icon: BarChart2, exact: false },
  { to: "/app/more", label: "More", icon: MoreHorizontal, exact: false },
];
            ...
            return (
              <li key={it.to}>
                <Link
                  to={it.to as any}
                  className={cn(
                    "flex flex-col items-center justify-center gap-0.5 py-2 min-h-[52px] text-[10px] font-medium transition-colors",
                    active
                      ? "text-primary"
